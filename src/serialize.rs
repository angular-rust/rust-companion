...
macro_rules! implement_handle_serialization {
    ($ty:ty) => {
        impl $crate::serialize::Serialize for $ty {
            fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
            where
                S: $crate::serialize::ser::Serializer,
            {
        ...
        impl<'de> Deserialize<'de> for $ty {
            ...
            where
                D: $crate::serialize::de::Deserializer<'de>,
            {
                let handle: $crate::Handle<$ty> =
                    $crate::serialize::Deserialize::deserialize(deserializer)?;
...
macro_rules! implement_typed_handle_serialization {
    ($ty:ty) => {
        impl<T: Serialize + DeserializeOwned> $crate::serialize::Serialize for $ty {
            fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
            where
                S: $crate::serialize::ser::Serializer,
            {
