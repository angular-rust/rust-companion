...
impl RawReceiver {
    ...
    fn recv_part(
        ...
        loop {
            let mut iov = [IoSliceMut::new(&mut buf[pos..])];
            ...
            let msg = recvmsg::<()>(self.fd, &mut iov, Some(&mut cmsgspace), flags)?;
...
impl RawSender {
    /// Connects a receiver to a named unix socket.
    pub fn connect<P: AsRef<Path>>(p: P) -> io::Result<RawSender> {
        let sock = UnixStream::connect(p)?;
        unsafe { Ok(RawSender::from_raw_fd(sock.into_raw_fd())) }
    }
